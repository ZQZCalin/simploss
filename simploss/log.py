...
def standard_log() -> LogFn:
    ...
    def update_fn(state, loss_val: chex.Array, params: optax.Params, grad: optax.Updates):
        ...
        cumulatives["grad/inner_sum"] += tree_util.inner(grad, grad_prev)
        cumulatives["grad/cosine_sum"] += tree_util.cosine(grad, grad_prev)

        metric = {
            "loss": loss_val,
            "grad/norm": tree_util.norm(grad),
            "grad/inner": tree_util.inner(grad, grad_prev),
            "grad/cosine": tree_util.cosine(grad, grad_prev),
        }
